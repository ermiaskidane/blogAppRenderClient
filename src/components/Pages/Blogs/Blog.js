...
const Blog = ({ match, history }) => {
  ...
  useEffect(() => {
    const getBlog = async () => {
      const { data } = await axios.get(
        `https://blogappserver-nay2.onrender.com/api/articles/read/${match.params.slug}`
      )
      setBlog(data)
    }
  ...
  useEffect(() => {
    ...
    const getBlogs = async () => {
      const { data } = await axios.get(
        'https://blogappserver-nay2.onrender.com/api/articles/all/'
      )
      const customData = []
      for (let i = 0; i < 8; i++) {
        ...
        <div className='blog__content--container'>
          <div className='blog__content--category'>
            ...
            <p className='created--time'>
              {moment(blog.createdAt).utc().format('DD/MM/YY')}
            </p>
            <div className='content--body'>
              <figure>
                <img
                  src={blog.image ? blog.image : `/images/img-3.jpg`}
                  alt='passive income'
                />
              </figure>
              <text cols='60' rows='8' style={{ whiteSpace: 'pre-wrap' }}>
                {blog.markdown}
              </text>
              {/* <p>{blog.markdown}</p> */}
            </div>
          ...
          <ul className='blog__content--lists'>
            ...
            {allBlogs.map((blog) => (
              <li className='content__list' key={blog._id}>
                ...
                <img
                  src={blog.image ? blog.image : `/images/img-2.jpg`}
                  alt='author'
                />
              </li>
            ))}
