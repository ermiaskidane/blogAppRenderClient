...
const UpdateArticle = ({ match, history }) => {
    ...
    if (userInfo) {
      const editData = async () => {
        const { data } = await axios.get(
          `https://blogappserver-nay2.onrender.com/api/articles/edit/${match.params.id}`
        )
        setTitle(data.title)
  ...
  const submitHandler = (e) => {
    ...
    const articleData = async () => {
      const blogPost = await axios.put(
        `https://blogappserver-nay2.onrender.com/api/articles/${match.params.id}`,
        data
      )
