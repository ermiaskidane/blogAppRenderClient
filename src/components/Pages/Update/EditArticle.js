...
const EditArticle = ({ history, match }) => {
    ...
    if (userInfo) {
      const getBlogs = async () => {
        const { data } = await axios.get(
          `https://blogappserver-nay2.onrender.com/api/articles/userBlogs?keyword=${keyword}&pageNumber=${pageNumber}&userArts=${userArts}`
        )
        setPage(data.page)
        setPages(data.pages)
  ...
  const DeleteHandler = async (b) => {
    console.log('::::::', b)
    // const uploadRes = await axios.delete(
    ...
    const removeBlog = await axios.delete(
      `https://blogappserver-nay2.onrender.com/api/articles/${b._id}`,
      config
    )
                ...
                <React.Fragment key={b._id}>
                  <li className='blogs__item'>
                    <Link className='blogs__item__link' to={`/blog/${b.slug}`}>
                      <figure
                        className='blogs__item__pic-wrap'
